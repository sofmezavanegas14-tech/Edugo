...
begin
  ...
  if not exists (select 1 from public.posts where id=p_post_id and (privacy='public' or user_id=v_user_id)) then raise exception 'POST_NOT_FOUND'; end if;
  ...
  if v_exists then
    ...
    insert into public.post_likes(user_id,post_id) values(v_user_id,p_post_id) on conflict(post_id,user_id) do nothing;
    liked:=true;
  end if;
